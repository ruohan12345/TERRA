...
class GLORYSWindowLinearDataset(data.Dataset):
    def __init__(
        self,
        root_full_path,
        wp_rank,
        ...
        if norm_type not in ("zs", "mm") or data_precision != "fp16":
            raise ValueError(
                "Only GLORYS window-linear zs/mm fp16 data is supported, "
                f"but got norm_type={norm_type}, data_precision={data_precision}"
            )

        rank_root = os.path.join(root_full_path, f"wp_rank_{wp_rank}")
        files_full = sorted(glob(os.path.join(rank_root, "*.pt")), key=sort_key)
        if not files_full:
            raise FileNotFoundError(f"No GLORYS split .pt files found under {rank_root}")
    ...
    def _load_tensor(self, index):
        tensor = torch.load(self.files_full[index], map_location="cpu", weights_only=True)
